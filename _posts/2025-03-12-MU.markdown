...
# Guided Distortion Module
Guided Distortion Module은 harmful prompts에 대해 정확하게 응답하기 위해 \( \theta_o \)로 표기되는 original LLM(pre-trained)을 용이하게 만드는 것을 목표로 한다.   

즉, unlearning process 이후 LLM에서 harmful knowledge를 식별하고 완화하는 것에 목적을 둔다.

harmful prompt-output pair \((x_f, y_f))\가 주어지면,  \(\psi _\theta (x, y_{<i}))\을 LLM \( \theta\)에 의한 token \(y_i)\의 예측 확률로 표현한다.