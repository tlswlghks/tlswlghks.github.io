...
## Experiments
본 논문에서는 다음을 통해 unlearned model 평가에 초점을 맞춘다.   

1. Performance on the Forget Set:
   - model은 forget set에 대해서 정확한 예측이 불가능하거나, 동일한 수준의 test set에 대한 성능이 감소해야 한다. 
2. Performance on the Retain Set:
   - retain set에 대해서 model의 성능이 감소하면 안된다.
3. Performance on General Downstream Tasks:
   - 일반적인 downstream task의 성능 평가는 unlearning 후의 model의 전반적인 capability에 대한 insights를 제공한다.
   - model의 성능은 unlearning하기 전과 비교하여 downstream task에서 성능 감소가 적을 것으로 예상한다.

### Approximate Retraining
- Approximate Retraining은 membership inference attacks에 영감을 받아 만든 LLM의 평가 방법이며, training data와 unseen data 사이의 성능 차이를 평가할 수 있다.   
- 본 논문에서는 retrained model이 trained data보다는 아니겠지만, unseen domain-specific data에 대해서 일관된 성능을 보일 것이라고 가설을 세운다.
- pre-train data 양과 unlearned data 양 사이의 상당한 불균형을 고려할 때, 우리는 unlearned data 분포 내에서 unseen data에 대한 retrained model의 성능이 vanilla model의 성능과 밀접하게 일치할 것으로 기대합니다.
  - unlearned data의 양이 pre-trained data의 양보다 상대적으로 훨씬 적기 때문에 unlearned data 분포 내에 unseen data를 제외한 retrained model의 성능은 기존의 model 성능과 크게 차이가 없다는 의미로 해석된다.
- 
